...
function validateName(name) {
    ...
    if (trimmed.length > ValidationRules.NAME_MAX_LENGTH) {
        return { valid: false, error: `名前は${ValidationRules.NAME_MAX_LENGTH}文字以内で入力してください`, value: trimmed };
    }

    if (ValidationRules.CONTROL_CHARS_PATTERN.test(trimmed)) {
        return { valid: false, error: '名前に制御文字を含めることはできません', value: trimmed };
    }
...
function validateDescription(description) {
    ...
    if (description.length > ValidationRules.DESCRIPTION_MAX_LENGTH) {
        return { valid: false, error: `説明は${ValidationRules.DESCRIPTION_MAX_LENGTH}文字以内で入力してください`, value: description };
    }

    if (ValidationRules.CONTROL_CHARS_PATTERN.test(description)) {
        return { valid: false, error: '説明に制御文字を含めることはできません', value: description };
    }
...
function validateTags(tagsString) {
    ...
    if (tags.length > ValidationRules.TAGS_MAX_COUNT) {
        return { valid: false, error: `タグは最大${ValidationRules.TAGS_MAX_COUNT}個までです`, value: tags };
    }

    for (const tag of tags) {
        if (tag.length < ValidationRules.TAG_MIN_LENGTH || tag.length > ValidationRules.TAG_MAX_LENGTH) {
            return { valid: false, error: `各タグは${ValidationRules.TAG_MIN_LENGTH}〜${ValidationRules.TAG_MAX_LENGTH}文字で入力してください`, value: tags };
        }

        if (ValidationRules.CONTROL_CHARS_PATTERN.test(tag)) {
            return { valid: false, error: 'タグに制御文字を含めることはできません', value: tags };
        }
    }
