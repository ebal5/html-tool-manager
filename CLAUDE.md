...
`tool`テーブルに`version`カラムを追加する必要があります。

```sql
-- 既存データベース用マイグレーション
ALTER TABLE tool ADD COLUMN version INTEGER NOT NULL DEFAULT 1;
```
...
1. アプリケーションを停止
2. 旧バージョンのコードをデプロイ
3. `ALTER TABLE tool DROP COLUMN version;`（任意 - カラムを残しても旧コードは動作する）
4. アプリケーションを起動
